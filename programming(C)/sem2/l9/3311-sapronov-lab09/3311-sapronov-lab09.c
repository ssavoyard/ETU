...
regions* make_region_node(char name[MSIZE]) {
    regions* region = NULL;
    region = (regions*)malloc(sizeof(regions));
    if (region != NULL) {
...
regions* find_region(regions_head* r_head, char str[MSIZE]) {
...
void add_regions_node(regions_head* r_head, regions* region) {
    r_head->cnt++;
...
void insert_region(regions_head* r_head){
    regions* node = NULL;
    char temp[MSIZE];
    printf("\nEnter new region name: ");
    fgets(temp, MSIZE, stdin);
    trim(temp);
    node = make_region_node(temp);
    r_head->cnt++;
    if (r_head->first == NULL) {
        node->prev = NULL;
        node->next = NULL;
        r_head->first = node;
        r_head->last = node;
    } else if (r_head->first == r_head->last) {
        node->prev = NULL;
        node->next = r_head->last;
        r_head->first = node;
        r_head->last->prev = node;
    } else {
        node->prev = r_head->last->prev;
        node->next = r_head->last;
        r_head->last->prev->next = node;
        r_head->last->prev = node;
    }
}
...
int main() {
    ...
    do {
        ...
        printf("\nEnter '1' if you want to add new city\nEnter '2' if you want to add new region\nEnter '0' to exit program\n");
        scanf("%d", &answer);
        getchar();
        clear_console();
        if (answer == 1) {
