...
private:
    struct Node {
        ...
        void* operator new(size_t size) {
            std::cout << "Allocating memory for Node of size " << size << std::endl;
            return ::operator new(size);
        }

        void operator delete(void* ptr) {
            std::cout << "Deallocating memory for Node" << std::endl;
            ::operator delete(ptr);
        }
    };
...
Set::Set() : head(nullptr), tag(++tag_counter) {
    std::cout << "Set " << tag << " created (default constructor)" << std::endl;
}

// Copy constructor
Set::Set(const Set& other) : head(nullptr), tag(++tag_counter) {
    std::cout << "Set " << tag << " created (copy constructor from Set " << other.tag << ")" << std::endl;
...
Set::Set(Set&& other) : head(other.head), tag(++tag_counter) {
    std::cout << "Set " << tag << " created (move constructor from Set " << other.tag << ")" << std::endl;
    other.head = nullptr;
}

// Destructor
Set::~Set() {
    std::cout << "Set " << tag << " destroyed (destructor)" << std::endl;
    while (head) {
        Node* temp = head;
        head = head->next;
        delete temp;
...
Set& Set::operator=(const Set& other) {
    if (this != &other) {
        std::cout << "Set " << tag << " assigned (copy assignment from Set " << other.tag << ")" << std::endl;
        while (head) {
            Node* temp = head;
            head = head->next;
            delete temp;
...
Set& Set::operator=(Set&& other) {
    if (this != &other) {
        std::cout << "Set " << tag << " assigned (move assignment from Set " << other.tag << ")" << std::endl;
        while (head) {
            Node* temp = head;
            head = head->next;
            delete temp;
...
void Set::add(char elem) {
    if (!contains(elem)) {
        std::cout << "Adding element '" << elem << "' to Set " << tag << std::endl;
...
Set Set::operator | (const Set& B) const {
    std::cout << "Performing union of Set " << tag << " and Set " << B.tag << std::endl;
    Set result;
...
Set Set::operator & (const Set& B) const {
    std::cout << "Performing intersection of Set " << tag << " and Set " << B.tag << std::endl;
    Set result;
    Node* temp = head;
    while (temp) {
...
Set Set::operator ~ () const {
    std::cout << "Performing complement of Set " << tag << std::endl;
    Set result;
...
void Set::randomFill() {
    ...
    std::cout << "Filling Set " << tag << " with random elements" << std::endl;
