...
//         if (right->count > 1) {
//             rightStr += "(" + std::to_string(right->count) + ")";
//         }
...
//     // Handle duplicates in sequence mode
...
//     current_count = 1; // Reset for next node
//     return *this;
// }
...
//     // For sequences, just decrement count if > 1
//     if (!is_set && q->count > 1) {
//         q->count--;
//         count--;
//         return true;
...
//         int num = random_int(1, 10); // Smaller range to increase duplicates
//         result.insert(num);
//     }
//     return result;
// }
...
// Node methods (empty as we don't need display)
void Node::Out(int row, int col) const {}
void Node::Display(int level, int col, int& max_level) const {}

// Tree display method (empty)
void Tree::Display() const {}
...
Tree generate_random_set() {
    Tree result(true);
    int size = random_int(1, SET_SIZE);
    while (result.size() < size) {
        int num = random_int(1, SET_SIZE);
        result.insert(num);
    }
    return result;
}
...
void execute_set_operations(Tree A, Tree B, Tree C, Tree D, Tree E) {
...
void execute_sequence_operations(Tree seq1, Tree seq2) {
    // Generate sequences
    seq1 = generate_random_sequence();
    seq2 = generate_random_sequence();
    ...
    std::merge(temp1.begin(), temp1.end(),
             temp2.begin(), temp2.end(),
             outinserter(merged, merged.begin()));
    ...
    Tree concatenated(false);
    for (int val : concat_result) {
        concatenated.insert(val);
    }
    
    // Erase range
    if (seq1.size() > 0) {
        int start = random_int(1, seq1.size()/2);
        int end = random_int(start+1, seq1.size());
...
int main() {
    ...
    Tree E;
        ...
        D = generate_random_set();
        E = generate_random_set();
        std::cout << "SET_SIZE " << SET_SIZE << std::endl; 
        start = getTime();
        execute_set_operations(A, B, C, D, E);
        std::cout << "meow" << std::endl; 
        execute_sequence_operations(A, B);
        finish = getTime();
        std::cout << "meow" << std::endl; 
        file << SET_SIZE << " " << std::chrono::duration_cast<std::chrono::microseconds>(finish - start).count() << std::endl;
